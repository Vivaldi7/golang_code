...
func createNote() (Note, error) {
	...
	resp, err := http.Post(baseUrl+createPastfix, "application/json", bytes.NewBuffer(data))
	if err != nil {
		return Note{}, err
	}
...
func getNote(id int64) (Note, error) {
	resp, err := http.Get(fmt.Sprintf(baseUrl+getPastfix, id))
	if err != nil {
	...
	if resp.StatusCode != http.StatusOK {
		return Note{}, errors.Errorf("failet to get note: %d", resp.StatusCode)
	}
