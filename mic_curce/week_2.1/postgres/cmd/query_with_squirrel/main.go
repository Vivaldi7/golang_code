...
func main() {
	...
	//Делаем запрос на выборку записей
	builderSelect := sq.Select("id", "title", "body", "created_at", "update_at").
		From("note").
		PlaceholderFormat(sq.Dollar).
		OrderBy("id ASC").
		Limit(10)
	...
	//Делаем запрос на полчение изменнеой записи
	...
	err = pool.QueryRow(ctx, query, args...).Scan(&id, &title, &body, &createdAt, &updateAt)
	if err != nil {
		log.Fatalf("faild to select notes: %v", err)
	}
